        ...
        do {
            ...
            for share in shares {
                ...
                case .localFolder(let url):
                    await scanLocalFolder(url: url, share: share)
                case .smb:
                    continue // SMB scanning to be implemented later.
                default:
                    continue
                }
            }
    ...
    private func scanLocalFolder(url: URL, share: SavedShareRecord) async {
        ...
        for case let fileURL as URL in enumerator {
            do {
                ...
                let key = mediaKey(for: share.id, path: fileURL.path, size: size ?? 0, mtime: mtime ?? 0)

                let guessed = guess(from: fileURL)
                let now = Date()
...
#if DEBUG
                print("Failed to index \(fileURL): \(error)")
#endif
            }
        }
    }
